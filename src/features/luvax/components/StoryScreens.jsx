...
export function StoryViewScreen({ viewport: vpProp }) {
  ...
  // Navigation is kept out of the progress updater. Calling navigate from inside
  // a setState updater fires a router update during render, which React warns
  // against; watching the value in its own effect closes the story cleanly.
  useEffect(() => {
    if (progress >= 100) navigate(ROUTES.FEED);
  }, [progress, navigate]);
